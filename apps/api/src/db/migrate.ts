...
export async function migrateDb(): Promise<void> {
    ...
    await db.execute(sql`CREATE INDEX IF NOT EXISTS idx_events_task_address ON indexed_events(task_address)`)
    await db.execute(sql`CREATE INDEX IF NOT EXISTS idx_events_event_name ON indexed_events(event_name)`)
    await db.execute(sql`CREATE INDEX IF NOT EXISTS idx_events_block_time ON indexed_events(block_time)`)
    await db.execute(sql`CREATE INDEX IF NOT EXISTS idx_events_signature ON indexed_events(signature)`)
    ...
    await db.execute(sql`CREATE INDEX IF NOT EXISTS idx_hist_final_status ON historical_tasks(final_status)`)
    await db.execute(sql`CREATE INDEX IF NOT EXISTS idx_hist_closed_at ON historical_tasks(closed_at)`)
    await db.execute(sql`CREATE INDEX IF NOT EXISTS idx_desc_task_address ON task_descriptions(task_address)`)
