import { pgTable, text, bigint, integer, timestamp, jsonb, index, uniqueIndex } from 'drizzle-orm/pg-core'
...
export const historicalTasks = pgTable(
    'historical_tasks',
    {
        ...
        /** Terminal status */
        finalStatus: text('final_status').notNull().$type<'Approved' | 'Cancelled' | 'Expired' | 'DisputeResolved'>(),
        /** Agent public key */
...
export const taskDescriptions = pgTable(
    ...
    (t) => [
        index('idx_desc_task_address').on(t.taskAddress),
    ],
)
...
export const taskTitles = pgTable(
    'task_titles',
    {
        /** Task PDA base58 address */
