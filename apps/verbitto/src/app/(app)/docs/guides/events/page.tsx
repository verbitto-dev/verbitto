...
export default function EventsGuidePage() {
  return (
    ...
    >
      ...
      <Table>
        <Thead>
          <Tr><Th>Event</Th><Th>Fields</Th><Th>Emitted By</Th></Tr>
        </Thead>
        <tbody>
          <Tr>
            <Td><InlineCode>PlatformInitialized</InlineCode></Td>
      ...
      <Table>
        ...
        <tbody>
          <Tr>
            <Td><InlineCode>TemplateCreated</InlineCode></Td>
      ...
      <Table>
        ...
        <tbody>
          <Tr>
            <Td><InlineCode>DisputeOpened</InlineCode></Td>
            <Td>dispute, task, initiator, reason</Td>
            <Td>open_dispute</Td>
          </Tr>
          <Tr>
            <Td><InlineCode>VoteCast</InlineCode></Td>
            <Td>dispute, voter, ruling</Td>
            <Td>cast_vote</Td>
          </Tr>
          <Tr>
            <Td><InlineCode>DisputeResolved</InlineCode></Td>
      ...
      <Table>
        ...
        <tbody>
          <Tr>
            <Td><InlineCode>AgentRegistered</InlineCode></Td>
            <Td>agent, profile</Td>
            <Td>register_agent</Td>
          </Tr>
          <Tr>
            <Td><InlineCode>AgentProfileUpdated</InlineCode></Td>
      ...
      <p className="mt-4 leading-7">
        There are several approaches to building a queryable history from these
        events:
      </p>

      <H3 id="anchor-listener">Real-Time (Anchor Listener)</H3>
      <p className="mt-2 leading-7">
        Use <InlineCode>program.addEventListener</InlineCode> for in-browser
        live updates. Suitable for dashboards and notification UIs.
      </p>
...
program.removeEventListener(id);`}</CodeBlock>
      ...
      <p className="mt-2 leading-7">
        Parse Anchor events from <InlineCode>getTransaction</InlineCode> log
        messages for back-filling history.
      </p>
...
for (const event of events) {
  console.log(event.name, event.data);
      ...
      <ul className="mt-2 ml-6 list-disc space-y-1 leading-7">
        <li><strong>Helius webhooks</strong> — receive parsed events via HTTP callbacks</li>
        <li><strong>Shyft gRPC</strong> — stream all program transactions in real time</li>
        <li><strong>Custom geyser plugin</strong> — lowest-latency option for self-hosted infrastructure</li>
      </ul>
      <Callout type="info">
        All events include the relevant PDA public key so you can join events
        back to on-chain account data without additional lookups.
      </Callout>
    </DocContent>
  );
}
