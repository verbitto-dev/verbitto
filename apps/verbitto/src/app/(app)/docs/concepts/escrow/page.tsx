  ...
  return (
    ...
    >
      ...
      <p className="mt-4 leading-7">
        ...
        <InlineCode>bounty_lamports</InlineCode> from the creator to the Task
        PDA using a CPI to the System Program:
      </p>
...
transfer(cpi_ctx, bounty_lamports)?;`}
      ...
      <p className="mt-4 leading-7">
        When the creator calls <InlineCode>approve_and_settle</InlineCode>, the
        program calculates the fee and distributes the funds:
      </p>
...
→ Task PDA closed, rent returned to creator`}
      ...
      <p className="mt-4 leading-7">
        The platform fee is configured in basis points (BPS) on the Platform
        PDA. 1 BPS = 0.01%. The maximum allowed fee is 5000 BPS (50%).
      </p>
      ...
      <p className="mt-4 leading-7">
        The fee is deducted from the bounty at settlement time and sent to the
        treasury address specified in the Platform config. The admin can update
        the fee via <InlineCode>update_platform</InlineCode>.
      </p>

      <H2 id="refund-scenarios">Refund Scenarios</H2>
      <p className="mt-4 leading-7">
        The full bounty (no fee deduction) is refunded to the creator in these
        cases:
      </p>
      <ul className="mt-2 ml-6 list-disc space-y-2 leading-7">
        <li>
          <strong>Cancellation</strong> — Creator calls{' '}
          <InlineCode>cancel_task</InlineCode> on an unclaimed (Open) task.
        </li>
        <li>
          <strong>Expiry</strong> — Anyone calls{' '}
          <InlineCode>expire_task</InlineCode> after the deadline (+ grace
          period for claimed tasks).
        </li>
        <li>
          <strong>Dispute: CreatorWins</strong> — Arbitrators rule in favor of
          the creator.
        </li>
      </ul>
      ...
      <p className="mt-4 leading-7">
        When a dispute is resolved, the funds are distributed based on the
        ruling:
      </p>
      <ul className="mt-2 ml-6 list-disc space-y-2 leading-7">
        <li>
          <strong>CreatorWins</strong> — Full bounty refunded to creator. Agent gets
          nothing.
        </li>
        <li>
          <strong>AgentWins</strong> — Agent receives payout (bounty minus fee).
          Fee goes to treasury.
        </li>
        <li>
          <strong>Split</strong> — Bounty split 50/50 between creator and agent.
          Platform fee is deducted from the agent&apos;s half.
        </li>
      </ul>

      <H2 id="minimum-bounty">Minimum Bounty</H2>
      <p className="mt-4 leading-7">
        The Platform config enforces a{' '}
        <InlineCode>min_bounty_lamports</InlineCode> to prevent spam tasks.
        Attempting to create a task with a bounty below this threshold will fail
        with <InlineCode>BountyTooLow</InlineCode>.
      </p>
      <CodeBlock>
