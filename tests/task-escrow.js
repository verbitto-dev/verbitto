...
describe('task-escrow', () => {
    ...
    const program = anchor.workspace.TaskEscrow;
    ...
    let platformPda;
    let _platformBump;
    let creatorCounterPda;
    let agentProfilePda;
    let voter1ProfilePda;
    let voter2ProfilePda;
    let voter3ProfilePda;
    ...
    before(async () => {
        // Derive platform PDA
        ;
        [platformPda, _platformBump] = PublicKey.findProgramAddressSync([Buffer.from('platform')], program.programId);
        [agentProfilePda] = PublicKey.findProgramAddressSync([Buffer.from('agent'), agent.publicKey.toBuffer()], program.programId);
        [voter1ProfilePda] = PublicKey.findProgramAddressSync([Buffer.from('agent'), voter1.publicKey.toBuffer()], program.programId);
        [voter2ProfilePda] = PublicKey.findProgramAddressSync([Buffer.from('agent'), voter2.publicKey.toBuffer()], program.programId);
        [voter3ProfilePda] = PublicKey.findProgramAddressSync([Buffer.from('agent'), voter3.publicKey.toBuffer()], program.programId);
        [creatorCounterPda] = PublicKey.findProgramAddressSync([Buffer.from('creator'), creator.publicKey.toBuffer()], program.programId);
    ...
    it('initializes the platform', async () => {
        await program.methods
            .initializePlatform(FEE_BPS, new BN(MIN_BOUNTY), new BN(VOTING_PERIOD), MIN_VOTES, new BN(MIN_VOTER_REPUTATION), new BN(CLAIM_GRACE_PERIOD))
            .accounts({
    ...
    describe('task lifecycle (happy path)', () => {
        ...
        it('creates a task with SOL escrow', async () => {
            ...
            [taskPda] = PublicKey.findProgramAddressSync([Buffer.from('task'), creator.publicKey.toBuffer(), taskIndex.toArrayLike(Buffer, 'le', 8)], program.programId);
            const deadline = Math.floor(Date.now() / 1000) + 3600; // 1 hour
            const _creatorBalanceBefore = await provider.connection.getBalance(creator.publicKey);
            await program.methods
                .createTask(title, Array.from(descHash), new BN(bounty), new BN(creatorTaskCount), new BN(deadline), new BN(50) // reputation reward
            )
                .accounts({
                // @ts-expect-error Anchor 0.31 type definitions don't recognize camelCase account names
                task: taskPda,
        ...
        it('agent submits deliverable', async () => {
            await program.methods
                .submitDeliverable(Array.from(deliverableHash))
                .accounts({
                // @ts-expect-error Anchor 0.31 type definitions don't recognize camelCase account names
                task: taskPda,
                platform: platformPda,
    ...
    describe('task cancellation', () => {
        ...
        it('creator cancels an open task and gets refund', async () => {
            ...
            await program.methods
                .createTask('Cancellable task', Array.from(Buffer.alloc(32, 3)), new BN(bounty), new BN(creatorTaskCount), new BN(deadline), new BN(10))
                .accounts({
                // @ts-expect-error Anchor 0.31 type definitions don't recognize camelCase account names
                task: taskPda,
                platform: platformPda,
    ...
    describe('task templates', () => {
        ...
        it('creates a template', async () => {
            ...
            await program.methods
                .createTemplate('Literature Review Template', Array.from(Buffer.alloc(32, 4)), new BN(0.5 * LAMPORTS_PER_SOL), { literatureReview: {} })
                .accounts({
                // @ts-expect-error Anchor 0.31 type definitions don't recognize camelCase account names
        ...
        it('creates a task from template', async () => {
            const taskIndex = new BN(creatorTaskCount);
            const [taskPda] = PublicKey.findProgramAddressSync([Buffer.from('task'), creator.publicKey.toBuffer(), taskIndex.toArrayLike(Buffer, 'le', 8)], program.programId);
            const deadline = Math.floor(Date.now() / 1000) + 3600;
            await program.methods
                .createTaskFromTemplate(new BN(0), // use default bounty from template
            new BN(deadline), new BN(30), new BN(creatorTaskCount))
                .accounts({
                // @ts-expect-error Anchor 0.31 type definitions don't recognize camelCase account names
                task: taskPda,
                template: templatePda,
    ...
    describe('dispute arbitration', () => {
        ...
        it('full dispute flow: open → vote → resolve (AgentWins)', async () => {
            ...
            const deadline = Math.floor(Date.now() / 1000) + 3600;
            await program.methods
                .createTask('Disputed task', Array.from(Buffer.alloc(32, 5)), new BN(bounty), new BN(creatorTaskCount), new BN(deadline), new BN(100))
                .accounts({
                // @ts-expect-error Anchor 0.31 type definitions don't recognize camelCase account names
                task: taskPda,
                platform: platformPda,
            ...
            await program.methods
                ...
                .rpc();
            await program.methods
                .submitDeliverable(Array.from(Buffer.alloc(32, 6)))
            ...
            // 2. Creator rejects
            await program.methods
                .rejectSubmission(Array.from(Buffer.alloc(32, 7)))
                // @ts-expect-error Anchor 0.31 type definitions
                .accounts({ task: taskPda, creator: creator.publicKey })
                // @ts-expect-error Anchor 0.31 type definitions don't recognize camelCase account names
                .signers([creator])
                .rpc();
            [disputePda] = PublicKey.findProgramAddressSync([Buffer.from('dispute'), taskPda.toBuffer()], program.programId);
            await program.methods
                .openDispute({ qualityIssue: {} }, Array.from(Buffer.alloc(32, 8)))
                .accounts({
                // @ts-expect-error Anchor 0.31
                task: taskPda,
                dispute: disputePda,
            ...
            for (const [voter, ruling, voterProfile] of [
                ...
                const [votePda] = PublicKey.findProgramAddressSync([Buffer.from('vote'), disputePda.toBuffer(), voter.publicKey.toBuffer()], program.programId);
                await program.methods
                    .castVote(ruling)
                    .accounts({
                    // @ts-expect-error Anchor 0.31
    ...
    describe('negative tests', () => {
        ...
        before(async () => {
            ...
            const deadline = Math.floor(Date.now() / 1000) + 3600;
            await program.methods
                .createTask('Negative test task', Array.from(Buffer.alloc(32, 10)), new BN(1 * LAMPORTS_PER_SOL), new BN(creatorTaskCount), new BN(deadline), new BN(50))
                .accounts({
                // @ts-expect-error Anchor 0.31
                task: taskPda,
                platform: platformPda,
        ...
        it('rejects task creation with bounty below minimum', async () => {
            ...
            const [badTaskPda] = PublicKey.findProgramAddressSync([Buffer.from('task'), creator.publicKey.toBuffer(), idx.toArrayLike(Buffer, 'le', 8)], program.programId);
            try {
                await program.methods
                    .createTask('Too cheap', Array.from(Buffer.alloc(32, 11)), new BN(1), // 1 lamport — below minimum
                new BN(creatorTaskCount), new BN(Math.floor(Date.now() / 1000) + 3600), new BN(10))
                    .accounts({
                    // @ts-expect-error Anchor 0.31
                    task: badTaskPda,
                    platform: platformPda,
        ...
        it('rejects task creation with deadline in the past', async () => {
            ...
            try {
                await program.methods
                    .createTask('Past deadline', Array.from(Buffer.alloc(32, 12)), new BN(0.1 * LAMPORTS_PER_SOL), new BN(creatorTaskCount), new BN(1000000), // way in the past
                new BN(10))
                    .accounts({
                    // @ts-expect-error Anchor 0.31
                    task: badTaskPda,
        ...
        it('rejects task creation with title > 64 chars', async () => {
            ...
            try {
                await program.methods
                    .createTask('A'.repeat(65), // too long
                Array.from(Buffer.alloc(32, 13)), new BN(0.1 * LAMPORTS_PER_SOL), new BN(creatorTaskCount), new BN(Math.floor(Date.now() / 1000) + 3600), new BN(10))
                    .accounts({
                    // @ts-expect-error Anchor 0.31
                    task: badTaskPda,
                    platform: platformPda,
        ...
        it('rejects submission by non-assigned agent', async () => {
            try {
                await program.methods
                    .submitDeliverable(Array.from(Buffer.alloc(32, 14)))
                    .accounts({
                    // @ts-expect-error Anchor 0.31
                    task: taskPda,
                    platform: platformPda,
        ...
        it('rejects reject_submission on non-submitted task', async () => {
            try {
                await program.methods
                    .rejectSubmission(Array.from(Buffer.alloc(32, 15)))
                    .accounts({
                    // @ts-expect-error Anchor 0.31
                    task: taskPda,
                    creator: creator.publicKey,
    ...
    describe('platform admin negative tests', () => {
        ...
        it('rejects task creation when platform is paused', async () => {
            ...
            const idx = new BN(creatorTaskCount);
            const [taskPda] = PublicKey.findProgramAddressSync([Buffer.from('task'), creator.publicKey.toBuffer(), idx.toArrayLike(Buffer, 'le', 8)], program.programId);
            try {
                await program.methods
                    .createTask('Paused task', Array.from(Buffer.alloc(32, 20)), new BN(0.1 * LAMPORTS_PER_SOL), new BN(creatorTaskCount), new BN(Math.floor(Date.now() / 1000) + 3600), new BN(10))
                    .accounts({
                    // @ts-expect-error Anchor 0.31
                    task: taskPda,
                    platform: platformPda,
        ...
        it('rejects update_platform with invalid fee (> 50%)', async () => {
            try {
                await program.methods
                    .updatePlatform(6000, // > 5000 (50%)
                new BN(MIN_BOUNTY), new BN(VOTING_PERIOD), MIN_VOTES, new BN(MIN_VOTER_REPUTATION), new BN(CLAIM_GRACE_PERIOD), treasury.publicKey)
                    .accounts({
                    // @ts-expect-error Anchor 0.31
                    platform: platformPda,
                    authority: authority.publicKey,
    ...
    describe('dispute negative tests', () => {
        ...
        before(async () => {
            ...
            const deadline = Math.floor(Date.now() / 1000) + 3600;
            await program.methods
                .createTask('Open task for dispute test', Array.from(Buffer.alloc(32, 30)), new BN(bounty), new BN(creatorTaskCount), new BN(deadline), new BN(10))
                .accounts({
                // @ts-expect-error Anchor 0.31
                task: taskPda,
                platform: platformPda,
        ...
        it('rejects dispute on non-submitted/rejected task (open)', async () => {
            const [disputePda] = PublicKey.findProgramAddressSync([Buffer.from('dispute'), taskPda.toBuffer()], program.programId);
            try {
                await program.methods
                    .openDispute({ qualityIssue: {} }, Array.from(Buffer.alloc(32, 31)))
                    .accounts({
                    // @ts-expect-error Anchor 0.31
                    task: taskPda,
                    dispute: disputePda,
        ...
        it('rejects dispute by non-party', async () => {
            ...
            await program.methods
                ...
                .rpc();
            await program.methods
                .submitDeliverable(Array.from(Buffer.alloc(32, 32)))
                .accounts({
                // @ts-expect-error Anchor 0.31
                task: taskPda,
                platform: platformPda,
            ...
            try {
                await program.methods
                    .openDispute({ qualityIssue: {} }, Array.from(Buffer.alloc(32, 33)))
                    .accounts({
                    // @ts-expect-error Anchor 0.31
                    task: taskPda,
                    dispute: disputePda,
